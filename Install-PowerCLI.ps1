...
function Install-PowerCLI {
    ...
    begin {
        ...
        # TLS 1.0 registry path
        $Tls1 = 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.'
        $Tls10_Client = $Tls1 + '0\Client'
        $Tls10_Server = $Tls1 + '0\Server'
        $Tls11_Client = $Tls1 + '1\Client'
        $Tls11_Server = $Tls1 + '1\Server'

        $Tls_Reg_Entries = @($Tls10_Client, $Tls10_Server, $Tls11_Client, $Tls11_Server)
        ...
        if ([Net.ServicePointManager]::SecurityProtocol -notlike '*Tls1[23]*') {
            ...
            Set-ItemProperty -Path 'HKLM:\SOFTWARE\Wow6432Node\Microsoft\.NetFramework\v4.0.30319' `
                -Name 'SchUseStrongCrypto' -Value '1' -Type DWord -Force -Confirm:$false
            ...
            #######################
            foreach ($Tls_Reg_Entry in $Tls_Reg_Entries) {
                if (Test-Path -Path $Tls_Reg_Entry) {
                    Set-ItemProperty -Path $Tls_Reg_Entry -Name 'DisabledByDefault' -Value '1' -Type DWord -Force `
                        -Confirm:$false
                    Set-ItemProperty -Path $Tls_Reg_Entry -Name 'Enabled' -Value '0' -Type DWord -Force `
                        -Confirm:$false
                }
            }
            #######################
            ...
            #######################
            $msg = 'This system did not have TLS 1.2 or above enabled. Please re-run the script so that the ' +
            'changes to enable TLS 1.2 and above take effect.'
            Write-Warning $msg
            Exit
        }
    ...
    process {
        ...
        #######################
        Write-Progress -Id 1 -Activity 'Checking for PowerCLI' `
            -CurrentOperation 'Checking if PowerCLI is installed in Program Files, wait 5-30 seconds'
        ...
        Try {
            ...
            if ($PowerCLIImportTest) {
                ...
                $msg = 'Something went wrong! Maybe you, maybe me. Does this computer have internet access and ' +
                "did you run as administrator?`r`nTry installing PowerCLI in offline mode (Procedure 3): " +
                'https://tinyurl.com/VMware-PowerCLI'
